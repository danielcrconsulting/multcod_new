...
Uses
  ...
  IMulticoldServer1, ADODB, SutypGer, InvokeRegistry, System.UITypes, Pilha, Vcl.FileCtrl, DBTables;
...
//    continue;
  ...
  if QueryDlg.GridPesq.Cells[4,i] = 'AND' then
    s := s + '*'
  else if QueryDlg.GridPesq.Cells[4,i] = 'OR' then
    s := s + '-';
  end;
s := s + ')';
...
//    Teste.LogIn(LogInRemotoForm.UsuEdit.Text, LogInRemotoForm.PassEdit.Text, ConnectionID);
    StrAux := (formGeral.HTTPRIO1 as IMulticoldServer).LogIn(LogInRemotoForm.UsuEdit.Text, LogInRemotoForm.PassEdit.Text, ConnectionID);
      ...
      If PodeIncluir(J, ArDFNRemoto[J].CodSis, ArDFNRemoto[J].CodGrupo) Then
        With AssisAbreRemotoForm.TreeView1.Items Do
          ...
          If ArDFNRemoto[J].CodGrupo < 0 Then
            AddChild(RelRoot,Format('%3.3d',[ArDFNRemoto[J].CodGrupo])+' - '+ArGrupoRemoto[I].NomeGrupo)
          Else
            AddChild(RelRoot,' '+Format('%3.3d',[ArDFNRemoto[J].CodGrupo])+' - '+ArGrupoRemoto[I].NomeGrupo);
          End;
    End;
