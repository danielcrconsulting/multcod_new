...
uses  SysUtils, StrUtils, Module1, DataMov_, RotGerais, frmLog_, frmPath_, SimulaMenu_, Consulta_, frmAgenda_, frmAgenda1_,
  frmMes_, AlterarManual_, Incluir_, frmBIN_, Alterar_Consulta_, ConsultaCon_,
  frmConvMes_, ConsultaCvMes_, Periodo_, PBar_, DataLimite_, atMoeda_, frmGeraMov_, Historico_, frmJuncao_, DataDeAte_,
  GrupoConAlt_, GrupoCon_, NovoClienteAlt_, frmBaixaBanco_, frmRelease_, NovoCliente_, frmAbout_, TipoConAdm_, frmMes1_,
...
begin
  ...
  PedeAlteracao.Caption := 'Consulta Conciliação';
...
begin
  ...
  gData2 := '';
  PedeAlteracao.Caption := 'Exclui Relatório';
  PedeAlteracao.Label1.Caption := 'Digite o Relatório';
  gOpcao := '';
  PedeAlteracao.ShowModal();
  if gOpcao='' then Exit;
//  if  not Conecta('ADM') then Exit;
...
//  RsDb := gBanco.OpenRecordset(sSql);
  ...
  RsTb.TableName := 'tbConvMes';
  RsTb.Filter := 'Relatorio = '#39''+gOpcao+''#39'';
  RsTb.Open;
...
//    Desconecta;
  Else
  if Application.MessageBox(PCHAR('Confirma Exclusão do Relatorio '+gOpcao), 'Excluir Registros', MB_YESNO)=IDYES then
    begin
    RsTb.Delete;
...
begin
  ...
  PedeAlteracao.Caption := 'Exclui BIN';
  PedeAlteracao.Label1.Caption := 'Digite o BIN';
  gOpcao := '';
  PedeAlteracao.ShowModal;
  if gOpcao='' then
    Exit;
...
//  RsDb := gBanco.OpenRecordset(sSql);
  ...
  RsTb.TableName := 'tbBIN';
  RsTb.Filter := 'BIN = '#39''+gOpcao+''#39'';
  RsTb.Open;
  ...
  if Application.MessageBox(PCHAR('Confirma Exclusão do BIN '+gOpcao), 'Excluir Registros', MB_YESNO)=IDYES then
    RsTb.Delete;
...
begin
  ...
  PedeAlteracao.Caption := 'Altera Grupo de Conciliação';
  PedeAlteracao.Label1.Caption := 'Código do Grupo';
  gOpcao := '';
  PedeAlteracao.ShowModal;
  if gOpcao <> '' then
    begin
...
begin
  ...
  PedeAlteracao.Caption := 'Exclui Grupo de Conciliação';
  PedeAlteracao.Label1.Caption := 'Grupo de Conciliação';
  gOpcao := '';
  PedeAlteracao.ShowModal;
  if gOpcao <> '' then
    begin
...
//    sSql := 'select * from tb_GrupoCon where cd_grupo_con = '#39''+gOpcao+''#39'';
    sSql := 'cd_grupo_con = '#39''+gOpcao+''#39'';
//    RsDb := gBanco.OpenRecordset(sSql);
    RsTb.Filter := sSql;
    RsTb.Open;
    ...
    while Clientes[I] <> '999999' do
      ...
      If Conecta(DbTemp, gDataPath+'\'+Clientes[I]+'\'+Clientes[I]+'.udl') then
        ...
        while not RsTemp.EOF do
          begin
          RsDB.Insert;
          VBtoADOFieldSet(RsDb, 'BIN', RsTemp.FieldByName('BIN').Value);
          RsDb.Post;
          RsTemp.Next;
          end;
